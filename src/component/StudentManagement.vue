...
const levelOptions = computed(() => {
  const set = new Set(students.value.map(s => s.academic_level).filter(Boolean));
  return Array.from(set);
});
...
const filteredStudents = computed(() => {
  ...
  return students.value.filter(stu => {
    ...
    return matchName && matchNumber && matchGrade && matchClass && matchLevel && matchWeak;
  }).map(stu => ({
    ...stu,
...
const loadStudents = async () => {
  ...
  try {
    ...
    // 调试输出
    console.log('加载到的学生数据:', data);
    // 兼容后端未返回 weak_subjects_list 字段的情况
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="stu in filteredStudents" :key="stu.id">
              ...
              <td class="px-4 py-2">{{ stu.major }}</td>
              <td class="px-4 py-2">{{ stu.class_name }}</td>
              <td class="px-4 py-2">{{ stu.academic_level_display }}</td>
              <td class="px-4 py-2">
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md relative overflow-y-auto max-h-[80vh]">
          ...
          <form @submit.prevent="handleAddStudent" class="space-y-4">
            ...
            <div>
              ...
              <input v-model="addForm.major" type="text" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500" />
            </div>
            <div>
