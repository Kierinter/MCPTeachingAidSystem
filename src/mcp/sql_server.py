...
DB_CONFIG = {
    ...
    "db": os.getenv("DB_NAME", "mcp_db"),
    "autocommit": True,  # 自动提交事务
    "connect_timeout": 10,  # 连接超时时间
    "max_allowed_packet": 16777216,  # 最大允许的数据包大小
    "charset": "utf8mb4",  # 字符集
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                print(f"数据库连接失败，已达到最大重试次数: {e}")
                raise
            print(f"数据库连接失败，正在进行第 {attempt + 1} 次重试...")
...
@mcp.tool()
async def query_data(sql: str, ctx: Context, limit: Optional[int] = 100) -> str:
    """
    执行只读SQL查询并返回结果
